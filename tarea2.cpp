...
int  checkMagicArray(int magic_square[8][8]){
	int cuadro[8][8]=magic_square;
	int r=1;
	int suma1=0,suma2=0,k=0,l=7;
	//Suma la primera fila
	for (int i=0;i<8;i++){
		suma1=suma1+cuadro[0][i];
	}
	//Suma la diagonal
	for (int i=0;i<8;i++){
		suma2=suma2+cuadro[i][i];
	}
	//Compara la diagonal con la primera fila
	if(suma1!=suma2){
			r=0;
			return r;
	}
	//Compara todas las filas con la primera
	for(int i=1;i<8;i++){
		suma2=0;
		for(int j=0;j<8;j++){
			suma2=suma2+cuadro[i][j];
		}
		//Si una de las filas es diferente retorna el valor 0
		if(suma1!=suma2){
			r=0;
			return r;
		}
	}
	//Compara todas las columnas con la primera  fila
	for(int i=0;i<8;i++){
		suma2=0;
		for(int j=0;j<8;j++){
			suma2=suma2+cuadro[j][i];
		}
		//Si una de las columnas es diferente retorna el valor 0
		if(suma1!=suma2){
			r=0;
			return r;
		}
